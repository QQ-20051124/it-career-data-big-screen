...
def main():
    ...
    print("\n" + "=" * 60)
    print("【无地域限制全量采集】前程无忧(北京入口→全量遍历)启动 - "
          + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    print("关键词固定：计算机")
    print("说明：已取消北京单一城市限制，自动遍历全部城市入口")
    print("=" * 60)
    ...
    print(f"\n[COLLECT] 前程无忧(北京入口→全量) 采集完成，原始汇总: {len(all_jobs)} 条")
    ...
    print(f"\n[DONE] 【无地域限制全量采集】前程无忧(北京入口→全量)完成 - "
